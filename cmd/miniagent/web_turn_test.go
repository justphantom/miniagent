...
// stepLLM returns a tool call on step 1 and text on step 2, with distinct per-step usage.
type stepLLM struct {
	mu    sync.Mutex
	calls int
}
...
// §3.5: web path emits step_usage events with incrementing step and usage sums matching the result.
...
// §4.6: tool_use/tool_result events carry the step field.
