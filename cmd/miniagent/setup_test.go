...
func TestProviderForListModels_ModelFlag(t *testing.T) {
	cfg := &miniagent.Config{
		Providers: []miniagent.ProviderConfig{
			{Name: "p1", ChatURL: "http://a/v1/chat/completions"},
			{Name: "p2", ChatURL: "http://b/v1/chat/completions"},
		},
	}
	model := "p2/m"
	f := &cliFlags{model: &model}
	p, err := providerForListModels(cfg, f)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if p.Name != "p2" {
		t.Errorf("got provider %q, want p2", p.Name)
	}
}

func TestProviderForListModels_DefaultsModel(t *testing.T) {
	cfg := &miniagent.Config{
		Providers: []miniagent.ProviderConfig{
			{Name: "p", ChatURL: "http://a/v1/chat/completions"},
		},
		Defaults: miniagent.DefaultsConfig{Model: "p/m"},
	}
	f := &cliFlags{}
	p, err := providerForListModels(cfg, f)
	if err != nil {
	...
	if p.Name != "p" {
		t.Errorf("got provider %q, want p", p.Name)
	}
}

func TestProviderForListModels_SingleProviderNoModel(t *testing.T) {
	cfg := &miniagent.Config{
		Providers: []miniagent.ProviderConfig{
			{Name: "p", ChatURL: "http://a/v1/chat/completions"},
		},
...
func TestProviderForListModels_MultiProviderNoModelErrors(t *testing.T) {
	cfg := &miniagent.Config{
		Providers: []miniagent.ProviderConfig{
			{Name: "p1", ChatURL: "http://a/v1/chat/completions"},
			{Name: "p2", ChatURL: "http://b/v1/chat/completions"},
	...
	f := &cliFlags{}
	if _, err := providerForListModels(cfg, f); err == nil {
		t.Error("expected error for multi-provider without -model/defaults.model")
	}
}
