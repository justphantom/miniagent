...
internal/miniagent/   核心库（零外部依赖，纯标准库）
  types.go            domain 类型：Message/Tool/Request/Response/Result/LoopHooks/LoopConfig
  loop.go             Run：ReAct 主循环
  loop_extra.go       callLLMWithDowngrade / callLLMOnce（含 panic 兜底）
  loop_tools.go       handleToolCalls / runToolsParallel / defaultShapeResult
  wire.go             OpenAI Chat Completions 序列化层（buildChatBody/parseChatResponse）
  client.go           ChatClient：非流式 + models 列表，重试/降级
  stream.go           StreamClient：流式 SSE
  stream_parse.go     parseSSE：SSE 帧解析与增量聚合
  context.go          FitHistory：超窗摘要 + 有损 fallback + 主动裁剪；NewCompaction
  ...
  events.go           NDJSON 事件编码（session/tool_use/tool_result/result/error/delta/model）
```

入口 `cmd/miniagent/main.go` 自上而下：**flag → config → Resolve → key → workdir/session → 合并项目规则 → Set\* 运行时覆盖 → buildLLM → buildTools → loopCfg → NewCompaction → buildHooks → Run → 落盘**。
...
for step in 1..iterLimit:
    ...
    resp, downgraded = callLLMWithDowngrade  # 调 LLM
    if ErrContextLength:                     # 撞 context 上限
        msgs = trimHistoryForContext(msgs)   # 收紧历史
        重试一次本步
    hooks.AfterLLM(step, resp)               # 开放缝②：用量记账/静默溢出判定
    累加 usage（全零则本地估算 fallback）
    hooks.OnBudget(step, total)              # 开放缝③：预算熔断
    if 无 tool_calls:
        appendMsg(最终文本+真实usage); return finishStop
    msgs = handleToolCalls(...)              # 执行工具并回灌
    if step == iterLimit:                    # 撞上限：注入总结请求再调一次
...
- **ErrContextLength 被动重试**：`trimHistoryForContext` 清 reasoning + 压 tool content 后重试一次，超则上抛——核心健壮性，防长会话崩溃。
- **撞上限总结**：迭代上限前一步若刚执行工具，注入内部 `summaryRequest`（`roleSystem`，不持久化）引导 LLM 输出最终文本。

## 4. 开放缝：`LoopHooks`

`types.go:LoopHooks` 是核心与外部能力的唯一缝口，所有字段可 nil：

| 钩子 | 时机 | 职责 | nil 行为 |
|---|---|---|---|
| `BeforeLLM` | 每步调 LLM 前 | 改写发给 LLM 的消息视图、收缩 transcript、注入记忆/RAG、提交持久化摘要、累加用量 | 透传原 transcript |
| `AfterLLM` | 每步响应后 | 用量记账、静默溢出判定（置下步 Force） | 不通知 |
| `OnBudget` | 累加 usage 后 | 预算熔断（`ErrBudgetExceeded`） | 回落内置 `MaxTotalTokens` 判定 |
...
`context.go:NewCompaction` 把整套压缩封装为一对 `(before, after)` 钩子。`before` 每步做 `applyCompactionBarrier` + `FitHistory`；`after` 采真实 usage 判静默溢出、置下步 `Force`。

**压缩屏障**：`applyCompactionBarrier` 定位最新一条 `Kind=="summary"` 消息，只把它及之后的消息进 context，之前的旧历史仍留 session 文件（机会性 rewrite 才真正丢弃）。

**FitHistory 流水线**（`context.go:139`）：
...
**静默溢出检测**（`overflow.go`）：provider 200 成功但实际已超窗的两类——Case2（`finishStop` 但 input > window）、Case3（`finishLength` 且 output=0 且 input ≥ 99% window）。`after` 钩子据此置下步 `Force`，撞 provider 400 前先压。

**context 超限识别**：`isContextLengthError` 用 24 条正则 + 3 条排除（防 throttling/rate-limit 误命中），状态门从仅 400 放宽到 `400||413`。

## 6. 工具系统

**内置工具**（7 个，`buildTools` 注册）：`read` / `write` / `edit` / `grep` / `glob` / `codemap` / `shell`，外加 `.miniagent/scripts.json` 定义的 `script_<name>`（复用 shell 安全策略）。
...
**ChatClient**（非流式，`client.go`）：`sync.Once` 保护端点懒解析；重试仅对瞬时故障（429/5xx + 网络错误）生效 `maxRetries=2` 次，退避 `retryBaseDelay*2^attempt`，`Retry-After` 头优先且受 `retryMaxDelay` 封顶。响应体上限 `maxChatBodyBytes=4MiB`。

**StreamClient**（流式 SSE，`stream.go`）：流式 client **不带总 Timeout**（会砍断长生成），总时长交 ctx 控制；注入的 client 若有 Timeout 则借用其 Transport 另造无 Timeout client。重试仅在 **pre-delta 阶段**（client.Do 失败或非 200）有效；进入 `parseSSE`（已 200、已流 delta）即不可撤回不重试。
...
**wire 层**（`wire.go`）：`chatMessage`/`chatToolCall` 与 domain `Message`/`ToolCall` 字段刻意重复，上层 domain 不绑死厂商 JSON 形状。`buildChatBody` 写入思考级别（默认字段 `reasoning_effort`，`ThinkingMapping` 可覆盖字段名与级别映射，跨供应商兼容），含请求体大小预估拦截防 OOM。`Message.Kind`/`Usage`/`IsError` 是 session 层标记，**绝不泄漏给 LLM**（`buildChatBody` 独立构造）。
...
**Set\* 运行时覆盖**：main 据 resolved 调 `SetMaxReadFileBytes` / `SetMaxShellOutputChars` / `SetMaxSessionBytes` / `SetSummaryMaxTokens` 等（`atomic` 保护并发，防 -race），把 config 值注入核心内置默认。
