...
3. **`pickMPG` 静默覆盖**（`resolve.go`）：模型 CW > run CW 时取较大者无 warn，操作者 `run.context_window` 意图被静默丢弃（如 run=128K 但模型=200K → 压缩阈值按 200K 算）。
...
## 参考
- `internal/miniagent/config/resolve.go`、`cmd/miniagent/main.go`
