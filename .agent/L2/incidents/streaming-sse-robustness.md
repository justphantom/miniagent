...
## 做法（commit `00038cf`，`internal/provider/openai/stream_parse.go` + `stream.go`）
...
## 参考
- `internal/provider/openai/stream.go`（`DoStream`）、`stream_parse.go`（`parseSse` / `apply` / `errStreamUnterminated`）
- commit `00038cf`
