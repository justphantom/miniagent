...
// 仍超裁到最近轮，再超报错（审查 v2 #8 + v3 #4）。返回 error 时 Run 应终止避免循环烧请求。
func compactIfOverWindow(ctx context.Context, llm *HTTPClient, cfg LoopConfig, msgs *[]Message, newMsgs *[]Message, logger *slog.Logger) error {
	...
	summarized, serr := compactWithSummary(ctx, llm, compModel, msgs, contextKeepRecent, newMsgs)
	if serr != nil {
		if logger != nil {
	...
	if estimateTokens(*msgs) > cfg.ContextWindow*4/5 {
	...
	if estimateTokens(*msgs) > cfg.ContextWindow*4/5 {
		return fmt.Errorf("history 超 context window（约 %d tokens）即使有损裁剪后仍超——终止以避免循环烧请求", estimateTokens(*msgs))
	}
	return nil
}

// summarizeMiddle 调 LLM 把中段 msgs 压成一段摘要文本（不带 tools）。返回经 summaryMaxChars
// 截断的摘要。复用 HTTPClient.Do；调用方据 error 回落有损压缩（审查 v2 #6）。
func summarizeMiddle(ctx context.Context, llm *HTTPClient, model string, msgs []Message) (string, error) {
	if len(msgs) == 0 {
	...
	return truncate(strings.TrimSpace(resp.Text), summaryMaxChars, "…[摘要已截断]"), nil
}
...
// 或 LLM 失败返回 error（调用方回落 compactHistory）。无中段可摘返回 (false, nil)。
func compactWithSummary(ctx context.Context, llm *HTTPClient, model string, msgs *[]Message, keepRecent int, newMsgs *[]Message) (bool, error) {
	rounds := splitRounds(*msgs)
	if len(rounds) <= 1+keepRecent {
		return false, nil // 无中段可摘
	}
	...
	if err := validateToolPairing(middle); err != nil {
		return false, fmt.Errorf("中段配对断裂，无法安全摘要：%w", err)
	}
	summary, err := summarizeMiddle(ctx, llm, model, middle)
	if err != nil {
		return false, err
	}
