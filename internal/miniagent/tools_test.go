...
func TestRunWithTimeout_RecoversPanic(t *testing.T) {
	r := runWithTimeout(context.Background(), time.Second, "测试", func(_ context.Context) ToolResult {
		panic("boom")
	})
	if !r.IsError {
		t.Fatalf("内部 goroutine 的 panic 应被 recover 成 IsError 结果: %+v", r)
	}
	if r.ExitCode != exitCodeNotSet {
		t.Errorf("ExitCode = %d, want exitCodeNotSet", r.ExitCode)
	}
}
