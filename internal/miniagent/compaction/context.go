...
func summarizeMiddle(ctx context.Context, llm miniagent.Doer, model, summarizerPrompt, previousSummary string, maxChars int, msgs []miniagent.Message) (string, miniagent.Usage, error) {
	...
	return miniagent.Truncate(strings.TrimSpace(resp.Text), maxChars, "…[摘要已截断]"), resp.Usage, nil
}
...
// 按 round 当前 estimateRoundTokens 与 tokenBudget 的比例缩放每条 tool content 字符数（复用 miniagent.TruncateHeadTail 头1/4+尾3/4）。
...
func shrinkRoundToolContents(round []miniagent.Message, tokenBudget int) []miniagent.Message {
	...
	for i, m := range out {
		if m.Role == miniagent.RoleTool && len(m.Content) > 0 {
			...
			newLen = max(1, newLen)
			out[i].Content = miniagent.TruncateHeadTail(m.Content, newLen, "…[tool 结果已压缩]")
		}
	}
	return out
}
...
func compactWithSummary(ctx context.Context, budget ContextBudget, msgs []miniagent.Message, keepRecent int) (out []miniagent.Message, summary miniagent.Message, usage miniagent.Usage, err error) {
	...
	// §P0-B：summaryMsg 显式打新戳 miniagent.NowMs()（不经 appendMsg）——防陈旧的关键触发点：新 Ts 使其前
	...
	summaryMsg := miniagent.Message{Role: miniagent.RoleUser, Kind: miniagent.KindSummary, Content: summaryPrefix + text, Ts: miniagent.NowMs()}
...
// （复用 miniagent.TruncateHeadTail 的头 1/4 + 尾 3/4），把思考模型超长 reasoning 的中段发散压掉、两端保留。
