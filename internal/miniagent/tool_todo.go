...
func TodoTools(l *TodoList) []Tool {
	create := Tool{
		Name:        "todo_create",
		...
		Call: func(ctx context.Context, args string) ToolResult {
			...
			if strings.TrimSpace(a.Subject) == "" {
				return ToolResult{IsError: true, Output: "subject 必填"}
			}
			it := l.Create(strings.TrimSpace(a.Subject), strings.TrimSpace(a.Description))
			b, _ := json.Marshal(it)
			return ToolResult{Output: string(b)}
		},
	}
	...
	update := Tool{
		...
		Call: func(ctx context.Context, args string) ToolResult {
			...
			if a.ID == 0 {
				return ToolResult{IsError: true, Output: "id 必填"}
			}
			status := TodoStatus(a.Status)
			if !validTodoStatus(status) {
				return ToolResult{IsError: true, Output: fmt.Sprintf("status %q 非法（pending|in_progress|completed）", a.Status)}
			}
			it, ok := l.Update(a.ID, status, strings.TrimSpace(a.Subject), strings.TrimSpace(a.Description))
			if !ok {
				return ToolResult{IsError: true, Output: fmt.Sprintf("任务 id %d 不存在", a.ID)}
			}
			b, _ := json.Marshal(it)
			return ToolResult{Output: string(b)}
		},
	...
	return []Tool{create, update, list}
}
