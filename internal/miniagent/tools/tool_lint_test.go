...
func TestLint_MissingSubcommand(t *testing.T) {
	res := LintTool(t.TempDir(), 0).Call(context.Background(), `{}`)
...
func TestLint_RejectsFixArg(t *testing.T) {
	got := LintTool(t.TempDir(), 0)
	for _, arg := range []string{"--fix", "-fix", "--write", "-w", "--enable-all", "--new", "--new-from-rev=HEAD"} {
...
func TestLint_AllowedSubcommandRuns(t *testing.T) {
	...
	dir := t.TempDir()
	res := LintTool(dir, 0).Call(context.Background(), `{"subcommand":"version"}`)
	if res.IsError {
...
func TestLint_BinaryMissingReturnsError(t *testing.T) {
	...
	res := LintTool(t.TempDir(), 0).Call(context.Background(), `{"subcommand":"version"}`)
	if !res.IsError {
