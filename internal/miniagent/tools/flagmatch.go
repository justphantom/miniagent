...
//   - 单破折号长名（go 风格 "-w"、"-modfile"）：只认与 longs 全等。go 生态前缀同名人多
//     （-work vs -w、-m vs -modfile），前缀外推必误杀。
//   - 双破折号长选项（git 风格 "--amend"）：全等或唯一前缀缩写（"--am" ≡ "--amend"，
//     git 原生语义）；缩写命中多于一个不判（保守放行，由更精确条目负责）。
//
// "=value" 后缀在匹配前统一剥离。
...
// 只做全等/等号粘合匹配，不前缀外推；长选项支持唯一缩写（--am ≡ --amend）。
var gitDeniedOptions = map[string][]optSpec{
	...
	"push": {
		{shorts: []string{"-f", "-d"}, longs: []string{"force", "force-with-lease", "delete", "mirror", "receive-pack", "exec", "repo"}, reason: reasonHistory},
	},
	"pull": {
		{longs: []string{"receive-pack", "exec", "repo"}, reason: reasonExec},
	},
	"*": {
