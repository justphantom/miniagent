...
// §P1-D：移除 volume-kill 后 ctx 超时语义仍生效（sleep > timeout → IsError + miniagent.ExitCodeNotSet + 超时提示）。
func TestShell_TimeoutStillReported(t *testing.T) {
	...
	if res.ExitCode != miniagent.ExitCodeNotSet {
		t.Errorf("超时 ExitCode = %d, want miniagent.ExitCodeNotSet", res.ExitCode)
	}
	if !strings.Contains(res.Output, "超时") {
