...
// 代码截断即丢准确性，给高于默认 policy.MaxToolResultInHistory 的额度（仍受 read 自身
// maxReadFileChars 输出上限约束）。miniagent.Tool.ResultLimit 取此值。
...
func runWithTimeout(ctx context.Context, timeout time.Duration, label string, fn func(ctx context.Context) miniagent.ToolResult) miniagent.ToolResult {
	if err := ctx.Err(); err != nil {
		return miniagent.ToolResult{IsError: true, Output: "已取消：" + err.Error()}
	}
	...
	go func() {
		defer func() {
			if r := recover(); r != nil {
				done <- miniagent.ToolResult{IsError: true, ExitCode: miniagent.ExitCodeNotSet, Output: label + "内部错误"}
			}
		}()
		done <- fn(runCtx)
	}()
	...
	case r := <-done:
		return r
	case <-runCtx.Done():
		return miniagent.ToolResult{IsError: true, Output: label + "超时或已取消：" + runCtx.Err().Error()}
	}
}
