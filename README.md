...
- `text`：`text` 事件里是增量片段（多次拼接成完整回答）；`result` 事件里是完整回答文本（达到 `maxIterations` 上限被强制终止时为空）
- `model`：本次调用使用的模型 id
...
达到 max iterations 上限（无最终文本，仍输出累计 usage）：

```jsonl
{"type":"result","model":"gpt-4o","input_tokens":8200,"output_tokens":1500,"steps":20}
```

> 注意：`result` 事件中 `text`、`input_tokens`、`output_tokens`、`steps` 即使为 0 也会出现键名（无 `omitempty`）。
...
| 码 | 含义 |
|----|------|
| 0 | 正常结束（含达到 `maxIterations` 上限、最终文本为空的截断场景） |
