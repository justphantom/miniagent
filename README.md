...
- **API**：`POST /api/turn`（NDJSON 流式，契约与 CLI stdout 逐字节一致；`session` 空则新建、非空则续跑）、`GET /api/sessions`（行含 `running`）、`GET /api/sessions/{id}`（历史重放）、`GET /api/sessions/{id}/live`（旁观进行中轮次）、`POST /api/sessions/{id}/stop`、`GET /api/events`（全局生命周期流）、`GET /api/models`；除公开探测的 `GET /api/whoami` 外全部走 `x-api-key` 鉴权。
