...
- 流式：默认走 SSE，文本以增量片段实时输出（`-stream=false` 回退非流式）
...
-permission string       权限模式：default / free（默认 default；-set-permission 也读此值）
...
- 目录权限：`miniagent/` 及其子目录为 `0o750`；数据文件（`.jsonl`、`.cur`、meta 文件）为 `0o600`
