...
- **界面**：左侧会话列表（点击载入最近 200 条历史；会话项可删除 ✕、显示最后 assistant 消息预览与运行中呼吸点），右侧聊天区为**多视图**——每个会话独立视图，切换只换显示不中断流（后台轮次继续渲染进隐藏视图，切回即见全部）。**markdown 渲染**（标题/粗体/斜体/代码/列表/引用/表格/删除线，XSS 实体转义保护），流式期间 300ms 节流增量渲染（thinking 推理流不再直出原始 md 语法），finish 时终稿渲染。**暗亮主题切换**（◐/◑，localStorage 记忆）。**智能滚动**：上翻不拽回 + 「↓ 新消息」浮动按钮。**长文本折叠**：>24 行自动折叠+展开。**代码块复制按钮**。**header 显示**当前会话 id、累计 token 量（in=N out=M，含 replay 历史）、版本号、模型名。
- **API**：`POST /api/turn`（NDJSON 流式，契约与 CLI stdout 逐字节一致；`session` 空则新建、非空则续跑）、`GET /api/sessions`（行含 `running`）、`GET /api/sessions/{id}`（历史重放）、`GET /api/sessions/{id}/live`（旁观进行中轮次）、`POST /api/sessions/{id}/stop`、`GET /api/events`（全局生命周期流）、`GET /api/models`、`GET/PUT /api/config`（配置管理：读取当前配置 secret 掩码、校验后写回文件并提示「需重启生效」）；除公开探测的 `GET /api/whoami` 外全部走 `x-api-key` 鉴权。
