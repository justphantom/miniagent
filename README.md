...
- `-model` 为空 → stderr 报错，退出码 1
- `$MINIAGENT_API_KEY` 为空 → stderr 报错，退出码 1
- stdin 为空 → stderr 报错 `stdin is empty`，退出码 1
- `-blocked-patterns` 非 JSON 数组 → stderr 报错，退出码 1

### 权限模式与工具注册

| 模式 | 行为 |
|------|------|
| `plan` | 只读。注册 `read_file`（仅当 `-workdir` 非空）+ `webfetch` |
...
精确替换文件中的一段文本。`old_string` 必须在文件中唯一出现。出现 0 次或多次都失败。
...
scope 含义：
- `chat`：仅当前 `-chat-id` 可见
- `project`：同 stateDir 的所有会话共享（单文件 `project.json`）
- `global`：所有会话共享（单文件 `global.json`）
...
- `sanitizeChatID` 把非 `[a-zA-Z0-9._-]` 字符替换为 `_`
...
LLM 调用（`POST /v1/chat/completions`）的重试规则：
...
- 单次退避上限 60s
- 响应 body 上限 1 MiB（恰好 1 MiB 不截断，超过则报错）
