...
`deploy/deploy.sh` 完成：创建系统用户/组、装 `/usr/local/bin/miniagent`、播种 `/opt/miniagent/config/miniagent.json`（首次从 `config.example.json` 复制，**启动前需编辑填入 provider key**；config 目录归服务用户所有，WebUI 配置页保存时可直接写回）、创建数据目录、渲染并安装 `miniagent.service`。**部署变量（workdir/config/user/group/session-dir）从 `deploy/.env.example` 读取，局域覆盖放 `deploy/.env`（git-ignored）。** 服务以 `-serve` 运行（WebUI），unit 含 `NoNewPrivileges`/`ProtectSystem`/`PrivateTmp` 硬化；agent 层 shell 仍无约束。
...
- **配置**（`web` 段）：`listen`（默认 `127.0.0.1:8787`）、`key`、`allowed_hosts`（Host 头白名单的额外条目，反代域名/外部 IP 场景）、`max_concurrent_turns`（同时运行的 web 轮次上限，默认 0=不限，溢出 429）。
...
- **多会话并发与断连语义**：轮次与请求连接解耦——**关闭标签页/断网/切换会话视图不会取消轮次**（已执行部分照常落盘），停止必须走界面「停止」按钮或 `POST /api/sessions/{id}/stop`（取消服务端轮次并保存已执行部分）；轮次受 web 默认 10min 超时与 `run.max_duration` 约束，服务器关闭时在途轮次取消并保存。不同会话的轮次并行执行；同一会话同时仅一个轮次（并发请求 409）。
- **多浏览器同步**：每打开的浏览器保持一条全局生命周期流（`GET /api/events`：`turn_started`/`turn_finished`/`session_deleted`/15s `ping`），会话列表与 running 状态点实时刷新；对进行中的会话发送得 409 时自动升级为**旁观模式**（`GET /api/sessions/{id}/live`：从事件 0 重放 + 实时续流，`live_end` 收尾），可实时观看其他窗口驱动的轮次，也可用停止按钮协作停止。
- **界面**：**IDE 风骨架**（左侧图标栏 + 会话面板 slide-in + 主区多 tab 视图），左侧会话列表按项目（workdir）分组、会话项可删除 ✕、显示最后 assistant 消息预览与运行中呼吸点；点击载入最近 200 条历史。右侧聊天区为**多视图**——每个会话独立视图，切换只换显示不中断流（后台轮次继续渲染进隐藏视图，切回即见全部）。**工具轨迹面板**（🔧 图标）：按步骤分组的卡片时间线（工具图标/耗时/步号），配 `step_usage` 事件的每步 token 明细；**token 用量可视化**：状态栏 per-session 累计指标 + 每轮 usage 进度条（对预算）；**目录选择器**（📁：`GET /api/tree` 服务端目录浏览，跳过隐藏目录与 symlink，500 条截断，最近目录记忆）。**markdown 渲染**（标题/粗体/斜体/代码/列表/引用/表格/删除线，XSS 实体转义保护），流式期间 300ms 节流增量渲染（thinking 推理流不再直出原始 md 语法），finish 时终稿渲染。**暗亮主题切换**（◐/◑，localStorage 记忆）。**智能滚动**：上翻不拽回 + 「↓ 新消息」浮动按钮。**长文本折叠**：>24 行自动折叠+展开。**代码块复制按钮**。
- **API**：`POST /api/turn`（NDJSON 流式，契约与 CLI stdout 逐字节一致；`session` 空则新建、非空则续跑）、`GET /api/sessions`（行含 `running`）、`GET /api/sessions/{id}`（历史重放）、`GET /api/sessions/{id}/live`（旁观进行中轮次）、`POST /api/sessions/{id}/stop`、`GET /api/events`（全局生命周期流）、`GET /api/models`、`GET /api/tree`（目录浏览，目录选择器用；**可枚举运行用户可读的任意绝对路径下的一级子目录名**——见 SECURITY.md「设计已知」）、`GET/PUT /api/config`（配置管理：读取当前配置 secret 掩码、校验后写回文件并提示「需重启生效」）；除公开探测的 `GET /api/whoami` 外全部走 `x-api-key` 鉴权。
- **streaming**：`run.stream` 配置控制是否发 `text_delta`/`reasoning_delta`（默认非流式，界面显示最终文本与工具事件）。
- **配置管理**（侧栏 `⚙ 配置` 按钮）：全字段表单（providers / defaults / run / compaction / web / session，60+ 字段分 6 个可折叠分组，支持 text/number/bool/duration 数组/Host 白名单）。编辑后校验通过即写回配置文件（原子写，0600，`O_NOFOLLOW` 防 symlink 劫持）；**变更后需重启 miniagent 服务生效**（运行期不热重载），页面保存后会明确提示是否需要重启。敏感字段（provider.key / web.key）在界面中掩码回显，保留时传占位符、修改时替换原文。
